...
| 8 | #31 Authentication foundation | *"changing branch, start implementing next issue (issue 3)"* + *"read latest request changes from my friend, fix it and test"* (prior turn) + the standing sub-instructions: record both review directions, then open a PR and add @copter549365 as reviewer, and test/re-implement on any bug. | The agent caught that issue #31 had been **closed as completed on the board but never implemented** (no auth code on `lab3-staging`), and reopened it before starting. Recorded both review directions: my reviewer's approval of PR #38 after the fixes (Direction A), and my substantive `COMMENTED` review of the partner's Admin User Management PR #50 (Direction B) — again verifying before filing (no `passwordHash` leak in their `serializeUser`, correct three-gate order), then raising one real point: neither reset-password nor deactivate revokes the target's `Session` rows unless `requireAuth` re-checks `isActive`/`mustChangePassword` per request. Then it built the auth foundation the whole sprint depends on: `bcryptjs` hashing, SHA-256 session tokens in the DB, the **three-gate lifecycle** (authenticate → password gate → authorize), `requireRole`, and the four `/api/auth/*` endpoints — with the security details the spec turns on: a byte-identical login failure for unknown-email/wrong-password (with a dummy-hash comparison so the two can't be timed apart), deactivation revealed only after the password verifies, and `HttpOnly`/`SameSite=Lax` cookies. It then **removed the `X-Requester-Id` mechanism entirely** (deleting `requesterContext.ts`, `RequesterProvider`, `RequesterSelection`, `/select`), rewired every Lab 2 route behind the session, and rebuilt the client: an `AuthProvider` reading `/api/auth/me`, Login and Change Password screens, and a role-specific shell (identity, role badge, logout). Every Lab 2 API and client test was migrated from the header to a session cookie / auth context. Proved UNIT-01/02/03/09, API-01…10, AUTHZ-01/02, REG-03/06, UI-01…13. Verified: server **99/99**, client **59/59**, both `tsc` clean, client build. |
| 9 | #31 Authentication — peer review response | *"ตอบสิ่งที่เพื่อน comment มาใน pr ล่าสุดให้หน่อย"* → *"บันทึก reviewer.md กับ ai-use.md ให้ด้วย"* | @copter549365 left three non-blocking questions on PR #39. I told the agent to answer them; it **verified each against the code before replying** rather than drafting a response from the review text. Two were real gaps and it fixed them: `PASSWORD_MIN` was declared independently in `ChangePassword.tsx` and `password.ts`, so it added **UI-31**, a test that reads both sources and asserts they match — converting "remember to change both" into a failing build, and throwing rather than passing silently if either stops being a literal; and `POST /api/auth/password` already re-verified the current password but nothing asserted it and `tests.md` had no row, so it added **API-39**, deliberately asserting the stored password is *unchanged* (old one still authenticates, proposed one does not) rather than only the 400. The third needed no change: it read `app.ts` and confirmed `POST /api/tickets` carries `requireRole("REQUESTER")` while `GET /api/tickets` deliberately does not, because ownership already scopes it and a role guard would 403 IT Staff when an empty list is the correct answer. It also volunteered a finding I had not asked about — the client validates only `MIN` while the server validates `MIN` and `MAX` — and documented it as a deliberate non-change instead of quietly widening scope. Then it recorded the full exchange in `reviewer.md` and backfilled the two partner PRs (#42, #45) that were missing from Direction B. Verified: server **100/100**, client **60/60**, tsc clean, and both new tests re-run by name to prove they execute rather than being skipped. `tests.md` 113 → 115. |
| 10 | #32 Requester regression and Public Comments | *"changing branch, start implementing next issue"* + the standing sub-instructions: record both review directions first, then open a PR and add @copter549365 as reviewer, and test/re-implement on any bug. | Much of the "re-guard every Lab 2 route behind the session" scope was already delivered in #31, so the agent confirmed that and focused on the new surface: the **Public Comments** endpoints and the **resolution signal**. It added `validateCommentBody` (trim, 1–2000, never whitespace-only, UNIT-04); `GET`/`POST /api/tickets/:id/comments` behind a role-aware access helper (a Requester reaches only their own ticket, IT Staff/Admin any — so a comment is visible to staff, AC-22, while a comment on a ticket you don't own is an **identical 404**, AUTHZ-08); and `POST /api/tickets/:id/resolution-signal` that records the timestamp and signer but **returns `currentStatus` unchanged** as proof it resolved nothing (AC-25), refusing IT Staff/Admin with a distinct 403. Comment bodies are stored and rendered as text, never HTML (BR-45). Client: a `PublicComments` panel (composer + chronological thread with author role) and a **"Problem appears resolved"** action behind a confirmation that becomes a pale chip while the status badge stays unchanged — and crucially **no Internal Notes region on the Requester's detail in any state** (AC-24, UI-16). The one Lab 2 test that asserted "no editable control on the detail" was scoped to the ticket-info panel, since the comment composer is legitimately editable. Proved UNIT-04, API-11…14, AUTHZ-06/08, and (already green) REG-01/02/06, UI-14…16. Verified: server **110/110**, client **63/63**, both tsc clean, client build. |
